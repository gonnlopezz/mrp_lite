...
import { Planificacion, ProcesoPlanificacion } from '../planificacion/planificacion';
...
export class PlanificacionesComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.isOrderContext = this.route.snapshot.url[0].path.includes('pedidos');
    ...
    loadEntityDetails(): void {
        ...
        if (this.isOrderContext) {
            this.pedidoService.get(id).subscribe((dataPackage: any) => {
                const order = <PedidoFabricacion>dataPackage.data;
                this.order = order;
                this.entityName = `Pedido de ${order.cliente?.razonSocial}`;
            ...
            this.workshopService.get(id).subscribe((dataPackage: any) => {
                ...
                this.entityCode = workshop.codigo;
            });
        }
    }
    ...
    loadPlanning(): void {
        ...
        const request$ = this.isOrderContext
            ? this.pedidoService.getPlannings(id)
            : this.workshopService.getPlannings(id);

        request$.subscribe({
            next: (dataPackage: any) => {
                this.planningProcesses = <ProcesoPlanificacion[]>dataPackage.data;
    ...
    generateTimelineChart(): void {
        ...
        this.planningProcesses.forEach((process, processIndex) => {
            ...
            process.planificaciones.forEach(planning => {
                ...
                const taskName = planning.tarea?.nombre || 'Tarea';
                const equipCode = planning.equipo?.codigo || 'S/E';
